...
The supplied W11 GLB contains one fused mesh rather than four independently transformable wheel objects. Drive mode now animates the actual tyre, rim, hub and brake-disc vertices around the measured front and rear axle centers while leaving the brake calipers fixed. The front wheel geometry also steers during turns. This preserves the source textures—including the rotating tyre sidewall markings—without replacing the W11 with a generic wheel model. The attached CFD overlay moves with the presentation rig, but this motion is not a moving-body CFD recomputation.
